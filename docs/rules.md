...
2. Rebuild the parsed command into a normalized shell-safe command string.
3. Load the rule profile named in the host config.
4. Apply blacklist checks first.
5. Apply whitelist checks second.
6. Only run the SSH command if all checks pass.
